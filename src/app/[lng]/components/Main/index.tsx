...
const articleKeys = ["codeQuality", "businessMindset", "restParadox", "readingDocumentation", "AIStandard"];
...
export default async function Main({language}: { language: TFunction }) {
    return (
        <main className="h-full w-3/5 max-lg:w-4/5 max-sm:w-full m-auto">
            ...
            <Articles
                title={language("articles.title")}
                subTitle={language("articles.subTitle")}
                scrollLeft={language("articles.scrollLeft")}
                scrollRight={language("articles.scrollRight")}
                items={articleKeys.map((key) => ({
                    key,
                    title: language(`articles.articles.${key}.title`),
                    text: language(`articles.articles.${key}.text`),
                }))}
            />
        </main>
    );
